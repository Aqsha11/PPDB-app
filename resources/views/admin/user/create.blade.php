                ...
                <div class="space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <x-text-input type="text" id="name" name="name" :value="old('name')" class="mt-1" placeholder="Masukkan nama lengkap..." required />
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Nama lengkap pengguna</p>
                            <x-input-error :messages="$errors->get('name')" class="mt-1.5" />
                        </div>
                        <div>
                            <x-input-label for="email" value="* Email" />
                            <x-text-input type="email" id="email" name="email" :value="old('email')" class="mt-1" placeholder="Masukkan alamat email..." required />
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Contoh: nama@domain.com</p>
                            <x-input-error :messages="$errors->get('email')" class="mt-1.5" />
                        </div>
                        <div>
                            <x-input-label for="password" value="* Password" />
                            <x-text-input type="password" id="password" name="password" class="mt-1" placeholder="Masukkan password..." required />
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Minimal 8 karakter</p>
                            <x-input-error :messages="$errors->get('password')" class="mt-1.5" />
                        </div>
                        <div>
                            <x-input-label for="password_confirmation" value="* Konfirmasi Password" />
                            <x-text-input type="password" id="password_confirmation" name="password_confirmation" class="mt-1" placeholder="Ulangi password..." required />
