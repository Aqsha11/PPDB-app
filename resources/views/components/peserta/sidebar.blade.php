@php
    ...
    $currentStep = 1;
    if ($peserta && $peserta->nama_lengkap && $peserta->pas_foto) $currentStep = 2;
    if ($peserta && $peserta->orangTua?->nama_ayah) $currentStep = 3;
    if ($peserta && $peserta->sekolahAsal?->nama_sekolah) $currentStep = 4;
    if ($pendaftaran && $pendaftaran->jalur_pendaftaran_id) $currentStep = 5;
    if ($pendaftaran && $pendaftaran->dokumenPendaftarans()->count() > 0) $currentStep = 6;
    if ($pendaftaran && $pendaftaran->status_pendaftaran !== 'draft') $currentStep = 7;
    ...
    if ($registrationComplete) {
        ...
        if ($currentStep >= 2 && $currentStep <= 6) {
            $items[] = ['label' => $stepLabels[$currentStep], 'route' => $stepRoutes[$currentStep], 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>', 'highlight' => true];
        }
    }
@endphp

<aside class="fixed inset-y-0 left-0 z-[70] w-[272px] bg-white dark:bg-slate-900 border-r border-gray-200/60 dark:border-slate-800 flex flex-col overflow-hidden transition-all duration-300 ease-in-out lg:translate-x-0"
